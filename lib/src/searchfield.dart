...
class _SearchFieldState<T> extends State<SearchField<T>> {
  ...
  void didUpdateWidget(covariant SearchField<T> oldWidget) {
    ...
    if (oldWidget.suggestions != widget.suggestions) {
      ...
      if (highlightIndex >= 0 && oldWidget.suggestions.isNotEmpty) {
        final previouslySelected = oldWidget.suggestions[highlightIndex];
        final newIndex =
            widget.suggestions.indexWhere((e) => e == previouslySelected);
        highlightIndex = newIndex >= 0 ? newIndex : -1;
      }
    }
